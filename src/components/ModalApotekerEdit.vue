    ...
    <div v-if="isOpen" class="fixed inset-0 flex items-center justify-center z-50 bg-gray-800 bg-opacity-75">
        <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
            <div class="bg-white p-6">
                <h2 class="text-xl font-semibold mb-4 text-black">Edit Data Apoteker</h2>
                <form @submit.prevent="submitForm">
                    ...
                    <div class="mb-4">
                        <label class="relative block overflow-hidden rounded-md border border-gray-200 px-3 pt-3 shadow-sm focus-within:border-blue-600 focus-within:ring-1 focus-within:ring-blue-600">
                            <input
                                type="password"
                                v-model="form.password_apoteker"
                    ...
                    <div class="mb-4">
                        <label class="relative block overflow-hidden rounded-md border border-gray-200 px-3 pt-3 shadow-sm focus-within:border-blue-600 focus-within:ring-1 focus-within:ring-blue-600">
                            <input
                                type="text"
                                v-model="form.permission_apoteker"
...
export default {
    ...
    watch: {
        apoteker(newValue) {
            console.log('New Apoteker Data:', newValue); // Log new apoteker data
            this.form = { ...newValue };
        }
    },
    methods: {
        ...
        async submitForm() {
            ...
            if (!this.form.id_apoteker) {
                console.error('id_apoteker is undefined');
                return;
            }
            try {
                const response = await axios.patch(`http://localhost:4000/Apoteker/${this.form.id_apoteker}`, this.form);
