    ...
    <div v-if="isOpen" class="fixed inset-0 flex items-center justify-center z-50 bg-gray-800 bg-opacity-75">
        <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
            <div class="bg-white p-6">
                <h2 class="text-xl font-semibold mb-4 text-black">Edit Data Obat</h2>
...
export default {
    ...
    watch: {
        Obat(newValue) {
            console.log('New Obat Data:', newValue); // Log new Obat data
            this.form = { ...newValue };
        }
    },
    methods: {
        ...
        async submitForm() {
            ...
            if (!this.form.id_obat) {
                console.error('id_obat is undefined');
                return;
            }
            try {
                const response = await axios.patch(`http://localhost:4000/Obat/${this.form.id_obat}`, this.form);
